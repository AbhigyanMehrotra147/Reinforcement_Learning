...
def choose_action(context: np.ndarray, epsilon: float = 0.1) -> int:
    if(random.uniform(0,1)<epsilon):
        # high is exclusive but low is inclusive
        return np.random.randint(low=0,high=4)
    else:
        return get_best_action(context=context)
...
def run_greedy(contexts: list, iterations: int,epsilon: int, reward_sampler: sampler)->np.ndarray:
    for i in range(iterations):
        context = random.choice(contexts)
        action = choose_action(context=context,epsilon=epsilon)
...
def run_epochs(epochs: int, all_contexts: list, reward_sampler: sampler, epsilon: float, one_epoch_iterations: int)->list:
    Users = copy.deepcopy(all_contexts)
    temp = copy.deepcopy(all_contexts)

    for i in range(epochs):
        run_greedy(contexts=Users,iterations=one_epoch_iterations,\
                   epsilon=epsilon,reward_sampler=reward_sampler)
        temp[0] += Users[0]
        temp[1] += Users[1]
        temp[2] += Users[2]
        Users = copy.deepcopy(all_contexts)
...
all_contexts=run_epochs(epochs=EPOCHS,all_contexts=all_contexts,reward_sampler=reward_sampler,epsilon=EPSILON,one_epoch_iterations=ONE_EPOCH)
