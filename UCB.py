...
"""Given a context function chooses an action to take using the UCB policy"""
def choose_action(context: np.ndarray, time_steps: int, exploration: float) -> int:
    max_ = 0
    arg_max = 0
    temp = 0
    for action in range(context.shape[1]):
        # Choose the action if the action is never taken:
        if(context[2][action] == 0) :
            return action
        else:
            temp = ucb_value(exploration=exploration,context=context,action=action,time_steps=time_steps)
            if(temp > max_):
                max_ = temp
                arg_max = action
    return arg_max
